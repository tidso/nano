...
yes = {'yes','y', 'ye', ''}
no = {'no','n'} 
colors_used = ['blue', 'orange', 'green', 'red', 'purple', 'brown', 'pink', 'grey', 'yellow', 'light blue']
        ...
        if i <= total_paths - 1:
            if i == 0:
                ...
                for j in range(number_of_bands):
                    ...
                    plt.plot(KPOINTS_BY_PATH[i], eigenvalues - E_FERMI, c=colors[j % len(colors)], alpha=0.9)
                ...
                ax.axhline(y=0, label='Fermi Level', alpha=0.75, color = 'gray', ls='--')
                i += 1
            else:
                ax = plt.subplot(gs[i]) #xlabel = x_labels[i]
